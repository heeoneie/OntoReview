...
const RISK_LEVEL_STYLES = {
  ...
  '주의': 'bg-zinc-800 text-white border border-zinc-600',
  '경고': 'bg-zinc-800 text-white border border-zinc-600',
  '위험': 'bg-zinc-700 text-white border border-zinc-500',
};
    ...
    <div className="bg-zinc-900 rounded-2xl border border-zinc-800 p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-bold text-white">{t('compliance.title')}</h3>
        </div>
        {!data && !loading && (
          <button
            onClick={onGenerate}
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            <span className={`px-3 py-1 rounded-full text-sm font-semibold ${RISK_LEVEL_STYLES[data.overall_risk_level] || 'bg-zinc-800 text-zinc-400'}`}>
              {t('compliance.' + (KO_RISK_KEY[data.overall_risk_level] || 'caution'))}
            </span>
            {data.report_date && (
              <span className="text-sm text-zinc-500">{data.report_date}</span>
            )}
          </div>
          ...
          {data.monitored_channels?.length > 0 && (
            <div className="space-y-1.5">
              <h4 className="text-sm font-semibold text-zinc-500 uppercase tracking-wide">{t('compliance.channelStatus')}</h4>
              {data.monitored_channels.map((ch, idx) => (
                <div key={idx} className="flex items-center justify-between bg-zinc-800 rounded-lg px-3 py-2 border border-zinc-700">
                  ...
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    {ch.risk_count > 0 && (
                      <span className="text-white font-medium">{t('compliance.riskCount')} {ch.risk_count}{t('compliance.count')}</span>
                    )}
                  </div>
                </div>
              ))}
              ...
              {Object.entries(data.risk_assessment).map(([key, val]) => (
                <div key={key} className="bg-zinc-800 rounded-lg p-3 border border-zinc-700">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`px-1.5 py-0.5 rounded text-sm font-semibold ${ASSESSMENT_LEVEL_STYLES[val.level] || 'bg-zinc-700 text-zinc-400'}`}>
          ...
          {data.risk_events?.length > 0 && (
            <div className="space-y-2">
              <h4 className="text-sm font-semibold text-zinc-400 flex items-center gap-1 uppercase tracking-wide">
              ...
              {data.risk_events.map((event, idx) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm font-semibold text-zinc-200">{event.category}</span>
                    {event.channel && (
                      <span className="text-sm text-zinc-500">[{event.channel}]</span>
                    )}
                    {event.affected_count && (
                      <span className="text-sm text-zinc-500">{t('compliance.affected')} {event.affected_count}{t('compliance.count')}</span>
                    )}
                  </div>
                  <p className="text-sm text-zinc-400">{event.description}</p>
                  {event.recommended_action && (
                    <p className="text-sm text-zinc-300 mt-1">→ {event.recommended_action}</p>
                  )}
                </div>
              ))}
            </div>
          ...
          {data.next_actions?.length > 0 && (
            <div className="bg-zinc-800 rounded-lg p-3 border border-zinc-700">
              <h4 className="text-sm font-semibold text-zinc-400 mb-2 uppercase tracking-wide">{t('compliance.nextActions')}</h4>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-zinc-800">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-sm font-semibold text-zinc-400 uppercase tracking-widest">{t('compliance.auditLog')}</span>
            </div>
